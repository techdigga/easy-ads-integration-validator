...
| `MAX033` | Requires the public initialized callback and checks its order relative to initialization. | High | Bounded common-caller ordering is recognized; unresolved or conflicting cross-method paths remain a `WARN` because syntax-only analysis cannot prove runtime execution order. | Subscribe to `MaxSdkCallbacks.OnSdkInitializedEvent` before calling `InitializeSdk()`. |
...
| Rule | Check | Default severity | Static-analysis limits | Remediation pattern |
| --- | --- | --- | --- | --- |
| `MAX070` | Checks interstitial and rewarded show calls for matching local or bounded method-wrapper readiness guards; app-open is ignored. | Medium | API-aware helper-method paths are recognized when the indexed source connects them to matching readiness methods; reflection and unresolved runtime dispatch remain advisory. | Guard fullscreen show calls with the matching MAX readiness check. |
...
| `MAX073` | Checks each used interstitial/rewarded format for terminal reload or signal evidence after hidden/load/display failure callbacks. | Medium | Bounded helper, scheduled, statically identifiable event, and literal recovery-flag paths are recognized; reflection, arbitrary event-bus delivery, and runtime dispatch remain advisory. | Reload or signal the next fullscreen load from every used format’s terminal paths. |
