...
The `levelplay-unity` profile provides static Unity LevelPlay mediation checks for SDK `9.0.0` and newer. Known SDK versions before `9.0.0` fail `LP002`; Beta 10 does not claim support for older SDKs.
...
| `LP011` | Custom Android manifest ownership and required ad permissions are structurally compatible. | High | Enabled custom-manifest ownership without a committed manifest, or missing required permission evidence, is high-confidence `FAIL`; deterministic Android build/post-process permission injection can satisfy `AD_ID`; unresolved build-time injection is `WARN`; stale disabled-manifest evidence is high-confidence `WARN`; generated manifest merge output is outside the boundary. | Commit the custom manifest when its Unity toggle is enabled, review `INTERNET`, `ACCESS_NETWORK_STATE`, and `AD_ID`, and keep any build-time permission injection statically recognizable. |
...
| `LP013` | One non-placeholder LevelPlay app key is present for Android and iOS, and visible initialization values do not conflict with settings. | Critical | Missing platform keys or visible settings/code conflicts are high-confidence `FAIL`; values resolved through simple assignment chains pass when aligned; unresolved initialization values are high-confidence `WARN`; malformed or unavailable settings remain low-confidence `UNKNOWN`. | Configure non-placeholder Android and iOS app keys in the recognized settings asset and keep visible initialization values aligned. |
...
`LP004` parses the recognized serialized settings assets instead of treating a filename or root marker as sufficient evidence. A LevelPlay mediation settings asset must contain a recognized settings root and nonblank `AndroidAppKey` and `IOSAppKey` fields. The separate `LevelPlayMediatedNetworkSettings.asset` and legacy mediated-network settings assets are also recognized as structurally valid settings without requiring app-key fields; app-key coverage remains the responsibility of `LP013`. The parser also validates known boolean settings such as `EnableIronsourceSDKInitAPI`, `EnableAdapterDebug`, and `EnableIntegrationHelper`; unknown serialized fields are tolerated for forward compatibility.
